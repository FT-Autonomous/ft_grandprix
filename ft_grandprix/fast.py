...
class Driver:
    ...
    def process_lidar(self, ranges):
        ...
        old = 0.0
        steering_angle = self.last_steering_angle * old + steering_angle * (1 - old)
        self.last_steering_angle = steering_angle
        # speed = self.SPEED # Make a speed function
        if abs(steering_angle) < 0.1 and ranges[0] > 0.5:
            speed = 7
        else:
            speed = min(2, self.SPEED * 5 * (1 - abs(steering_angle) / np.pi))
